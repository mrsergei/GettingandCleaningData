...
- First it checks to see if current directory where it is running contains dirtectory `UCI HAR Dataset`. If not, it downloads the compressed data file into local directory and unzips it   
...
- Scipt combines `train` and `test` data for each set of files - measurements (`X_*.txt`), activity IDs (`y_*.txt`) and subect IDs (`subject_*.txt`)
- It replaces activity IDs with human readbable activity labels in activity vector(`activity`)
- It adds human readbale column names for actvitiy vector(`activity`), subject vector (`subject`) and feature columns (`featData`)
- It selects only feature data columns that correspond to mean and standard deviation variables
- It combines activity, subject and reduced feature data set into one called `featData` for furtehr processing:   
```
> head(featData[1:5])
...
```
- Using `reshape2` librbay fucntion melt, it generates long data format with activity, subject, variable and value:   
```   
> head(featDataLong)
...
```  
- Using `reshape2` library fucntion dcast it calculates mean of earch variabel per subject per activity and generates long data format for the averages: 
```  
